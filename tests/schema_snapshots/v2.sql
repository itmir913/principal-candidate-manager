-- PCM 스키마 지문 v2 (생성 당시 앱 버전 0.2.19)
--
-- 자동 생성 파일. 손으로 고치지 말 것 — tests/schema_freeze.rs 가 실제 스키마와 대조한다.
-- 이 파일이 존재하는 순간 v2 스키마는 동결이다.
-- migrations/v2/*.sql 을 고치면 이 테스트가 깨진다. 그때는 파일을 맞추지 말고
-- 새 버전(migrations/v3/ + V3_FRAGMENTS)을 추가하고 SCHEMA_VERSION을 올려라.
--- SCHEMA FINGERPRINT ---
...
CREATE TRIGGER trg_prevent_update_closed_application BEFORE UPDATE ON applications BEGIN SELECT RAISE(ABORT, 'Cannot update application: round is CLOSED. Only excluded/excluded_reason/department_name may change.') WHERE (SELECT status FROM rounds WHERE id = OLD.round_id) = 'CLOSED' AND ( OLD.student_id != NEW.student_id OR OLD.track_id != NEW.track_id OR OLD.round_id != NEW.round_id OR OLD.abandoned != NEW.abandoned ); SELECT RAISE(ABORT, 'Cannot update application: round is FINALIZED. Only abandoned 0->1 and department_name are permitted.') WHERE (SELECT status FROM rounds WHERE id = OLD.round_id) = 'FINALIZED' AND ( OLD.student_id != NEW.student_id OR OLD.track_id != NEW.track_id OR OLD.round_id != NEW.round_id OR (OLD.abandoned = 1 AND NEW.abandoned = 0) OR OLD.excluded != NEW.excluded OR OLD.excluded_reason IS NOT NEW.excluded_reason ); END;
