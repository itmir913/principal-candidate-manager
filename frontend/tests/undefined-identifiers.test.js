 ...
 *
 * 무엇을 잡나: **템플릿이** 참조하는 이름 중 그 파일에 정의가 없는 것.
 *
 * **스크립트 본문은 보지 않는다.** 13_frontend_pitfalls §4 의 "훑는 방법"은
 * `_ctx.<name>` **과 스크립트의 스코프 밖 식별자**를 함께 훑으라고 적었는데, 여기는
 * 앞쪽 절반만 구현했다. 그래서 정작 그 문서가 든 두 사례
 * (`cancelDeptEdit` ↔ `cancelEdit` 사고, 2026-09-21 의 AreasTab import 누락)는
 * **둘 다 스크립트 본문이라 이 스캐너가 잡지 못한다** — 렌더 테스트가 잡는다.
 * (감사에서 그 둘을 되살려 확인했다. 처음엔 "전부 이 유형"이라 적었는데 거짓이었다.)
 *
 * 그럼에도 값이 있는 이유: 렌더 테스트는 **픽스처가 도달시킨 영역만** 본다.
 * `v-if` 뒤에 숨은 템플릿 참조는 여기서만 잡힌다.
 */
