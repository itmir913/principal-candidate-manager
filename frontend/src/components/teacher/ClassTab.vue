            ...
            >
              ...
              <td style="padding: 13px 20px;">
                ...
                <!-- 이 목록은 `teacherGetApplications()` 를 인자 없이 불러 **전 라운드**를
                     ...
                     주석은 반드시 **여는 태그 밖**에 둔다. 속성 사이에 넣으면 주석의
                     `-->` 안에 있는 `>` 가 여는 태그를 닫아 버려서, **그 뒤의 속성이
                     전부 텍스트가 된다.** 글자가 새는 건 증상일 뿐이고 본질은
                     `:key` 와 `class` 바인딩이 조용히 사라지는 것이다 — 즉 이 주석이
                     지키려는 대상 자체가 무력화된다(2026-09-22 실제로 그랬다). -->
                <div
