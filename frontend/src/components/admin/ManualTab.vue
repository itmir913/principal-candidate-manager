...
const roundGuides = [
  ...
  },
  {
    ...
    note: '✓ "미선발"과 "포기"는 다릅니다. 미선발 처리는 라운드가 종료(CLOSED)된 동안 관리자가 명시적으로 선발하지 않기로 결정하는 것으로, 사유를 반드시 기록하며 정원 집계에 영향을 주지 않습니다(결격·서류 미비·정원 미달 등 구체적 사유는 기록된 사유 텍스트로 구분합니다). 포기는 마감(FINALIZED) 후 추천된 학생이 진학을 철회하는 것으로, 다음 라운드에 자리가 반환되며 되돌릴 수 없습니다. 라운드를 다시 열면 확정된 추천이 전부 초기화되므로, 결격자 한 명 때문에 재개방하지 말고 미선발 처리 기능을 사용하십시오.',
    warning: null,
  },
  {
    id: 'finalized',
