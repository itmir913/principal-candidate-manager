...
const roundGuides = [
  ...
  },
  {
    ...
    note: '✓ 졸업생 담당은 관리자가 별도로 생성한 계정(0학년 0반)으로 로그인하여 졸업생 지원자를 등록합니다.',
    warning: null,
  },
  {
    id: 'close',
    ...
    items: [
      ...
      '"종료하기" 버튼을 누르기 전, 학급별 입력 확정 현황이 표시됩니다. 미확정 학급이 있어도 종료 자체는 가능하지만, 미확정 학급에서 입력이 완전히 끝났는지 반드시 담임교사와 확인하세요.',
  ...
  },
  {
    ...
    items: [
      ...
      '학생별 "추천 확정"도 모집단위 순위를 건너뛸 수 없습니다. 같은 모집단위에 아직 추천되지 않은 상위 순위 지원자가 있으면 거부됩니다. 단 동점자끼리는 서로 막지 않으며(관리자 선택 보장), 상위 지원자가 이미 추천됐거나 제외 처리된 경우에도 막지 않습니다.',
      '결격·서류 미비 등으로 상위 순위 지원자를 건너뛰어야 한다면, 해당 학생의 행에서 "제외 처리" 버튼을 누르고 사유를 입력하십시오. 제외된 지원자는 자동 추천 후보와 수동 추천 순서 가드에서 모두 빠지므로 다음 순위 지원자를 바로 추천할 수 있습니다. "제외 해제"로 언제든 되돌릴 수 있습니다. 포기(다음 항목 참고)와 달리 정원 집계에는 영향을 주지 않습니다.',
      '라운드가 마감(CLOSED)된 동안에는 순위의 기준이 되는 설정을 바꿀 수 없습니다 — 재학생 우선 설정(대학·모집단위)과 학생의 재학/졸업 구분이 그렇습니다. 저장된 순위가 마감 시점 기준으로 계산된 값이기 때문입니다. 바꾸려면 라운드를 다시 열고 고친 뒤 다시 마감하십시오(마감 시 순위가 재계산됩니다). 정원 변경은 마감 중에도 가능합니다.',
    ...
    note: '✓ "제외"와 "포기"는 다릅니다. 제외는 라운드가 종료(CLOSED)된 동안 결격·서류 미비 등으로 이번 라운드 추천 대상에서 빼는 것으로, 사유를 반드시 기록하며 정원 집계에 영향을 주지 않습니다. 포기는 마감(FINALIZED) 후 추천된 학생이 진학을 철회하는 것으로, 다음 라운드에 자리가 반환되며 되돌릴 수 없습니다. 라운드를 다시 열면 확정된 추천이 전부 초기화되므로, 결격자 한 명 때문에 재개방하지 말고 제외 기능을 사용하십시오.',
    warning: null,
  },
  {
    id: 'finalized',
