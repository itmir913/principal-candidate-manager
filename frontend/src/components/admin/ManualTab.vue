...
const setupSteps = [
  ...
  { num: 5, title: '프로그램 제목 (선택)',  desc: '학교 이름이나 전형 구분을 지정합니다. 프로그램을 두 개 운영할 때 서로 구분됩니다.' },
  { num: 6, title: '두 프로그램 운영 (선택)', desc: '인원 제한이 있는 대학과 없는 대학을 포트를 나눠 각각 운영합니다.' },
]
...
const roundSteps = [
  ...
  { num: 4, title: '추천 확정 (관리자)',     desc: '자동 계산된 순위를 검토하고 대학별 추천자를 확정합니다. 대학 전체 순위와 모집단위별 순위를 전환해 볼 수 있습니다.' },
  { num: 5, title: '라운드 마감 (관리자)',         desc: '추천 확정이 완료되면 라운드를 최종 마감합니다. 마감 후에는 학과명 외에는 변경할 수 없습니다.' },
]
...
const setupGuides = [
  ...
  },
  {
    ...
    title: '대학 설정',
    where: '대학 설정 탭',
    desc: '전형요소 기초 데이터 업로드로 자동 생성된 대학·모집단위를 확인합니다. 대학마다 카드 한 장에 정원·추천인원·잔여인원과 모집단위 목록이 함께 표시됩니다. 대학별 전체 정원과 재학생 우선 여부를 설정하고, 각 모집단위의 정원 및 전형요소별 배점을 입력합니다.',
    note: '✓ 대학이 많으면 화면 위쪽 검색칸에 대학명이나 모집단위명을 넣어 좁힐 수 있습니다. 모집단위를 직접 추가할 수도 있지만, 석차연명부 업로드 시 자동 생성되므로 권장하지 않습니다.',
  ...
  },
  {
    ...
    title: '프로그램 제목 (선택)',
    where: '설정 탭',
    desc: '프로그램 제목과 설명을 지정합니다. 지정하면 로그인 화면·사이드바·브라우저 탭과 담임교사 화면에 표시됩니다. 지정하지 않으면 기본 문구가 그대로 표시되므로 건너뛰어도 됩니다.',
    note: '✓ 인원 제한이 있는 전형과 없는 전형을 각각 다른 프로그램으로 운영할 때 특히 유용합니다. 제목에 학교 이름, 설명에 전형 구분(예: "인원 제한 없는 대학")을 넣으면 담임교사도 어느 프로그램에 접속했는지 알 수 있습니다.',
    warning: '⚠ 작업 표시줄 아이콘에 표시되는 이름은 프로그램을 다시 시작한 뒤에 바뀝니다. 화면의 제목은 저장 즉시 반영됩니다.',
  },
  {
    step: 6,
    title: '두 프로그램 동시 운영 (선택)',
    where: 'pcm/config.json',
    desc: '인원 제한이 있는 대학과 없는 대학은 관리 방식이 달라, 한 프로그램에 섞으면 전형요소 설정이 서로 발목을 잡습니다. 프로그램을 서로 다른 폴더에 두 벌 설치하고 각 폴더의 pcm/config.json 에서 "port" 를 다르게 지정하면(예: 8080 / 8081) 두 개를 동시에 운영할 수 있습니다. 포트가 같으면 나중에 켠 쪽이 시작하지 못합니다.',
    note: '✓ 운영 예시 — 인원 제한이 있는 대학(8080)은 전형요소를 모두 등록해 순위가 정확하게 나오도록 하고, 인원 제한이 없는 대학(8081)은 대학·모집단위와 내신 정도만 설정합니다. 정원 경쟁이 없어 세부 전형요소가 필요 없으므로, 담임교사는 학생이 어느 대학에 지원하는지만 입력하면 됩니다.',
    warning: '⚠ 두 프로그램은 각자의 pcm 폴더에 데이터를 따로 저장합니다. 담임교사가 어느 쪽에 접속했는지 헷갈리지 않도록 설정 탭에서 제목과 설명을 다르게 지정하십시오. 백업도 두 폴더를 각각 받아야 합니다.',
  },
]
...
const roundGuides = [
  ...
  },
  {
    ...
    title: '라운드 마감 — 관리자',
    desc: '추천 확정이 모두 끝나면 라운드를 최종 마감합니다. 마감 후에는 학과명 외에는 변경할 수 없습니다.',
    items: [
      ...
      '마감하면 담임교사의 [라운드 결과] 화면에 우리 반 학생의 순위·총점·추천 여부가 공개됩니다. 담임교사는 그 화면에서 "이 라운드 CSV"·"전체 결과 CSV" 버튼으로 학생 1명당 1행짜리 요약 파일을 내려받아 문자 일괄발송에 쓸 수 있습니다.',
      '학과명은 예외로 마감 후에도 고칠 수 있습니다. 점수 산출에 쓰이지 않고 명단 관리용이기 때문입니다. [라운드] 탭의 지원자·결과 표에서 학과명을 눌러 수정하세요. 담임교사는 [라운드 결과] 화면에서 최종 마감된 라운드에 한해 자기 반 학생의 학과명을 고칠 수 있습니다(마감 처리 중인 라운드는 관리자만 가능). 수정 내역은 감사 기록에 전후 값이 남습니다. 대학·모집단위는 마감 후 바꿀 수 없습니다.',
    ],
    note: '✓ 마감된 라운드의 데이터는 삭제되지 않으며 언제든지 다시 조회할 수 있습니다. 담임교사 화면에서 초록색 행은 추천 확정, 붉은색 행은 미선발 또는 추천 포기입니다.',
    warning: null,
  },
]
</script>
