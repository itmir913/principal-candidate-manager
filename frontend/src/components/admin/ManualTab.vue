...
const roundGuides = [
  ...
  },
  {
    ...
    items: [
      ...
      '결과 탭의 "자동 추천 확정" 버튼을 누르면 두 단계로 추천을 일괄 확정합니다. 1단계로 모집단위 순위순으로 모집단위 잔여 정원까지 채우고, 2단계로 대학 전체 정원이 있으면 그 결과를 대학 전체 순위로 다시 정렬해 상위까지만 남깁니다. 정원이 무제한이면 해당 단계의 컷이 걸리지 않습니다. 버튼을 다시 눌러도 이미 확정된 추천은 유지됩니다.',
