...
const roundGuides = [
  ...
  },
  {
    ...
    items: [
      ...
      '학생별 "추천 확정"도 모집단위 순위를 건너뛸 수 없습니다. 같은 모집단위에 아직 추천되지 않은 상위 순위 지원자가 있으면 거부됩니다. 단 동점자끼리는 서로 막지 않으며(관리자 선택 보장), 상위 지원자가 이미 추천됐거나 지원을 포기한 경우에도 막지 않습니다. 상위 지원자를 건너뛰어야 할 사정이 있다면 먼저 그 학생을 지원 포기 처리하십시오.',
