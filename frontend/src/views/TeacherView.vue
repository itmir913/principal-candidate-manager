    ...
    <main class="p-6 max-w-4xl mx-auto">
      ...
      <template v-if="activeTab === 'results'">
        <div class="bg-white border rounded-lg overflow-hidden">
          ...
          <div v-if="resultsLoading" class="px-4 py-8 text-center text-sm text-gray-400">
            불러오는 중...
          </div>

          <div v-else-if="teacherResults.length === 0" class="px-4 py-8 text-center text-sm text-gray-400">
...
const resultsByStudent = computed(() => {
  const map = new Map()
  for (const r of teacherResults.value) {
...
async function loadResults() {
  ...
  try {
    teacherResults.value = await teacherGetResults()
  } catch {
...
async function switchTab(key) {
  activeTab.value = key
  if (key === 'results' && teacherResults.value.length === 0 && !resultsLoading.value) {
    await loadResults()
  }
}
