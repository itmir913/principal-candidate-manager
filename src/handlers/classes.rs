...
mod tests {
    ...
    async fn insert_class(pool: &sqlx::SqlitePool, grade: i64, class_no: i64) {
        let hash = bcrypt::hash("pass1234", 4u32).unwrap();
        sqlx::query(
    ...
    async fn upsert_existing_class_without_password_keeps_old_hash() {
        ...
        let old_hash: String =
            sqlx::query_scalar("SELECT password_hash FROM classes WHERE grade = 1 AND class_no = 1")
                ...
                .await
                .unwrap();
        // update teacher name only, no password
        upsert_class(
            State(make_state(pool.clone())),
            Path((1i64, 1i64)),
    ...
    async fn delete_class_no_students_succeeds() {
        let pool = create_test_pool().await;
        insert_class(&pool, 1, 1).await;
        delete_class(State(make_state(pool.clone())), Path((1i64, 1i64)))
            .await
            .unwrap();
    ...
    async fn delete_class_with_students_blocked() {
        ...
        .await
        .unwrap();
        let res = delete_class(State(make_state(pool)), Path((1i64, 1i64))).await;
