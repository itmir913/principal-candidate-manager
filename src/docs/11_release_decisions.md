...
**결정**: 출시된 스키마 버전의 조각 파일은 동결한다. 스키마 변경은 새 버전
(`migrations/v2/` + `SCHEMA_VERSION` 상향)으로만 한다.

**구현**: `src/db.rs`(`SCHEMA_VERSION`, `MIGRATION_FRAGMENTS`),
`tests/schema_freeze.rs`, `tests/schema_snapshots/v{N}.sql`

**배경**: 출시 전에는 반대 규칙이었다 — 스키마 변경을 `migrations/v1/` 조각에 직접
반영하고 DB를 재생성했다. 사용자가 없으니 마이그레이션 관리 비용만 드는 일이었다.
출시로 그 전제가 깨졌다. 현장 DB는 v1 조각으로 만들어져 있고 `PRAGMA user_version`으로
자기를 1이라고 알린다. 조각 파일만 고치면 새 DB와 기존 DB의 구조가 갈라지는데 두 DB
모두 user_version은 1이라, **앱에는 그 차이를 감지할 수단이 아예 없다**. 상위 버전
DB를 막는 `SchemaTooNewError` 방어선도 버전 번호가 같으면 무력하다.

**강제 수단**: 버전별 스키마 지문(`sqlite_master` 전체 + `user_version`을 주석·공백
정규화 후 직렬화)을 `tests/schema_snapshots/v{N}.sql`에 고정한다. 조각을 고치면
`released_schema_versions_are_frozen`이 깨지고, 실패 메시지가 새 버전 추가 절차를
안내한다. 스냅샷 생성은 파일이 **없는** 버전에 한해서만 허용된다
(`PCM_WRITE_SCHEMA_SNAPSHOT=1`) — 기존 지문은 환경변수로도 덮어쓰지 않는다.

**앱 버전은 강제 대상이 아니다**(소유자 결정, 2026-08-05). 관리 대상은 스키마 버전
하나다. `Cargo.toml`의 앱 버전은 릴리즈 산출물 이름을 정할 뿐(`publish.yml`) DB 호환성
판정에 쓰이지 않으므로, 스키마 변경과 묶어 강제할 실익이 없다. 새 버전 스냅샷 헤더에
생성 당시 앱 버전을 남기는 것은 기록용이지 검사 항목이 아니다.

**뒤집기 전 확인**: "지문이 깨졌으니 스냅샷을 다시 뜨자"는 유혹이 이 결정의 주적이다.
스냅샷 재생성은 배포된 DB와의 구조 불일치를 덮는 행위지 고치는 행위가 아니다.
스키마를 되돌릴 수 없는 상황이라면 스냅샷이 아니라 버전을 올려라.
