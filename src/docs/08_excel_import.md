...
| 열 순서 무관 | 값은 **헤더 이름**으로 찾는다(`col_map` + `require_cols` + `get_col`). 열 순서가 템플릿과 달라도, 쓰지 않는 열이 끼어 있어도 정상 처리된다. 열 인덱스 직접 참조 금지 — `tests/excel_column_order.rs` 가 전 경로를 대조한다 |
| 이름 대조 | 학생 매칭은 학년·반·번호(졸업생·외부양식 일부는 학생코드)로 한다. 파일의 `이름` 이 다르면 **warning + 가져오기 완료** — 행이 밀린 파일을 알리되 개명·표기 차이로 전체 거부하지는 않는다 |
...
- **동작**: upsert (student_code 기준 SELECT-후-UPDATE/INSERT 분기, `students.rs`) — DELETE 없음. tx 안·단일 커넥션에서 실행되므로 SQL 네이티브 `ON CONFLICT DO UPDATE`와 결과 동등, TOCTOU 없음
...
- **COMPOSITE 선택 헤더**: `대학명`, `모집단위명` (numeric_table과 동일하게 **선택 사항**이다 —
  `require_cols`는 `범주`·`점수`만 강제한다(`area_data.rs`). 두 열을 생략하면
...
- **CLOSED 또는 FINALIZED** 상태 라운드가 존재하면 **409 Conflict** 반환 (`areas.rs`)
...
- 트리거 `trg_prevent_base_data_delete_for_applied`는 **BEGIN DELETE만** 막는다
  (`008-applications.sql`). 같은 파일 주석이 밝히듯 `INSERT OR REPLACE`는 내부 DELETE에
...
## 시트 선택과 숨김 행·열 (F-018 / S-05)

`excel.rs::parse_xlsx_all_rows`는 **첫 번째 시트만** 읽는다(`wb.sheet_names().first()`). 시트 **이름은 보지 않는다.**

- 앞에 표지·안내 시트가 있으면 필수 열을 찾지 못해 **400으로 거부**된다(fail-closed).
- 그러나 앞 시트가 우연히 같은 헤더를 가지면 **의도하지 않은 시트를 조용히 import** 한다.
- **숨긴 행·열도 그대로 읽는다.** "이 학생은 행을 숨겨서 제외했다"는 방식은 통하지 않는다 —
  제외하려면 행을 **삭제**해야 한다.

두 규칙 모두 테스트로 고정돼 있다(`tests/audit_invariant_excel_cases.rs`).

---

## 측정값의 도메인 범위 — 시스템이 정하지 않는다 (S-04)

`numeric_table.threshold`와 `base_data.value`에는 **형식 검증만** 있다(유한·±10억·소수 5자리).
"봉사시간은 0 이상", "석차는 1~9등급" 같은 **도메인 상·하한은 두지 않는다.**

의도된 설계다. 학교마다 전형요소와 그 척도가 다르고, PCM은 그 규칙을 학교가 직접 정의하도록
만든 시스템이다. 시스템이 특정 도메인을 못 박으면 그 학교에서 쓸 수 없다.
따라서 **값의 타당성은 점수표를 설계하는 관리자의 책임**이며, 시스템은 형식과 일관성만 강제한다.
