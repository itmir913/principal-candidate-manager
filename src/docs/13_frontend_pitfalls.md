...
- 픽스처가 도달시키지 못하는 화면 영역 전체. `v-if` 뒤에 있으면서 픽스처가 그 조건을
  만들지 못하면 그 영역은 통째로 검사 밖이다. 자동 마운트하는 화면 **대다수는
  "오류 0건"만** 보고, "무엇이 그려졌는지"까지 단언하는 것은 일부다.
  어느 화면이 어디까지 보는지는 `smoke-render.test.js` 의 `RENDER_EVIDENCE` 와
  `NEEDS_PROPS` 를 읽어라 — 여기에 목록이나 개수를 옮겨 적으면 낡는다(CLAUDE.md 규칙 9).
  큰 화면일수록 마운트 직후 그려지는 양이 적어 공백이 크다.
