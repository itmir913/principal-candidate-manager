...
**2026-09-21 이후로는 잡힌다.** `frontend/tests/smoke-render.test.js` 가 모든 화면을
마운트하고, [라운드] 화면은 결과 표까지 눌러 본다. 이 유형의 변이 4건(마운트 시 함수명
오타 / 템플릿이 없는 이름 참조 / computed 가 없는 함수 호출 / import 이름 오타)이
전부 검출되는 것을 확인했다 — **네 경우 모두 `npm run build` 는 통과한다.**
다만 **클릭해야 도달하는 핸들러** 안의 오타는 여전히 잡히지 않는다.
