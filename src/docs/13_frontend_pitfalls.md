...
드러나는** 종류다. 프론트 자동 검증은 세 겹이다 — `vitest`(`.vue` 밖의 순수 모듈),
`tools/oracle/front_check.mjs`(파생 계산 대조·소스 가드), 그리고 **스모크 렌더**
(`frontend/tests/smoke-render.test.js`, 2026-09-21 도입). 스모크는 모든 화면을 실제로
마운트해 "열리기는 하는가"만 본다. **외관·레이아웃·페인트는 여전히 단언하지 않는다** —
여기 적힌 함정 대부분이 그 종류이고, 그래서 한 번 밟은 것은 문서로 남긴다.
...
**여전히 잡지 못하는 것**: 클릭해야 도달하는 핸들러 **본문** 안의 오타
(정적 검사는 템플릿 참조만 보고, 스모크는 그 핸들러를 부르지 않는다).
