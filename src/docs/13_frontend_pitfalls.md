...
드러나는** 종류다. 프론트 자동 검증은 다섯 겹이다 — `vitest`(`.vue` 밖의 순수 모듈),
...
브라우저가 `<tr>`의 배경을 다시 칠하지 않는 경우가 있다.

같은 표의 학생 구분 행은 한 번도 안 깨졌는데, 그 행이 배경을 **`<td>`에 두었기**
때문일 가능성이 크다. 다만 그 행은 `colspan` 한 줄짜리라 **높이가 변하지 않는 행**
이기도 해서, 두 변수가 분리되지 않았다 — 단정하지 못한다.
...
배경이 `<tr>`에 있던 시절에는 문제가 없었다. 그때 호버는 `<td>`에
`background-color: rgba(15, 23, 42, 0.06)` 을 줬는데, **반투명이라 아래 `<tr>`의
의미색이 비쳐 색조가 남았다.**

그런데 §1을 적용해 배경을 `<td>`로 옮기면 사정이 달라진다. 이제 호버가 바꾸는
`background-color`가 **행 색을 담고 있는 바로 그 속성**이라, 덮어쓰는 순간 의미색이
통째로 사라진다. §1만 적용하고 §2를 빠뜨리면 호버한 행에서 추천 확정·미선발 구분이
없어진다.
...
### 배경을 진하게 하면 그 위 텍스트 대비를 다시 재라

흰색과 구분되게 하려고 배경을 한 단계 올리면(`-50` → `-100`) 그 위 텍스트의 명암비는
**반대로 떨어진다.** 실제로 이 프로젝트에서 상태 텍스트가 3.0:1 대로 내려가 WCAG AA
(4.5:1)를 밑돌았고, 텍스트를 `-700`/`-800` 계열로 함께 내려서 맞췄다.

호버 음영까지 얹힌 상태로 재야 한다 — 6% 어둡게만 해도 명암비가 0.5 안팎 더 떨어진다.
...
eslint도 타입 검사도 없다. 당시에는 백엔드 테스트·오라클·vitest 중 어느 것도 `.vue`
런타임을 실행하지 않았다.

**2026-09-21 이후로는 두 겹으로 잡는다.** 둘은 역할이 다르다.

- `frontend/tests/undefined-identifiers.test.js` — `.vue` 전 파일을 `vue/compiler-sfc`
  로 컴파일해 **템플릿이** 참조하는 이름 중 그 파일에 정의가 없는 것을 찾는다.
  **실행하지 않으므로 `v-if` 뒤도 본다.** 다만 **스크립트 본문은 보지 않는다** —
  아래 "훑는 방법"의 절반만 구현한 것이고, 위 `cancelDeptEdit` 사고와 2026-09-21 의
  AreasTab import 누락은 **둘 다 스크립트 본문이라 이 스캐너가 아니라 렌더 테스트가
  잡는다.**
- `frontend/tests/smoke-render.test.js` — 실제로 마운트한다. 실행되는 경로만 보지만,
  정적 분석이 못 보는 것(로더 안에서 난 오류, 렌더 중 TypeError)을 잡는다.
  이 저장소의 로더는 전부 `try/catch` 로 오류를 화면에 그리므로 **렌더된 글자에서도**
  오류 문구를 찾는다 — 그 판정이 없던 동안 `getClasses` → `getClassesTypo` 변이가
  통째로 빠져나갔다.

확인한 변이: 마운트 로더 함수명 오타 / 템플릿이 없는 이름 참조(`v-if` 뒤 포함) /
computed 가 없는 함수 호출 / import 이름 오타 / 렌더 중 TypeError.
**전부 `npm run build` 는 통과한다.**

**여전히 잡지 못하는 것**
- 클릭해야 도달하는 핸들러 **본문** 안의 오타(정적 검사는 템플릿 참조만 보고,
  렌더 테스트는 그 핸들러를 부르지 않는다).
- 픽스처가 도달시키지 못하는 화면 영역 전체. `v-if` 뒤에 있으면서 픽스처가 그 조건을
  만들지 못하면 그 영역은 통째로 검사 밖이다. 실측(2026-09-21): 자동 마운트하는 화면
  **19개**(`.vue` 25개 중 props 를 요구하는 5개와 `App.vue` 제외 — App.vue 는 스모크
  glob 4개 어디에도 안 걸린다) 가운데 `RENDER_EVIDENCE` 로
  "무엇이 그려졌는지"까지 단언하는 것은 4개뿐이고, 나머지는 "오류 0건"만 본다.
  **미검증 표면 1·2위**는 `RoundsTab`(1400줄인데 마운트 렌더 73자)과
  `AreasTab`(1336줄에 135자 — 점수표·기준표 편집 영역 전체 미도달)이다.
  (`DialogHost` 와 상세 모달은 전용 테스트로 닫았다.)

- 스크립트 본문의 미정의 식별자 중 **마운트 경로 밖**의 것.

### 소스 텍스트로 배선을 지키려 하지 마라

이 저장소는 그 실패를 **여섯 번** 겪었다 —
F-014(오라클 3c) 1회, F-013(오라클 3d) 2회, 정원 가드 카운트 2회, `destructive-confirm` 1회.
매번 "정규식을 넓히면 된다"·"세는 방향을 뒤집으면 된다"고 고쳤고 매번 다시 뚫렸다
(중첩 괄호, `??`, **주석 처리**, 변수 추출, 가드 없는 폼 추가,
**공용 컴포넌트를 안 쓰는 폼**, 표식 없는 폼).

배선은 **행동으로만** 지킨다 — 화면을 마운트해 버튼을 누르고 **상태**를 본다.
소스 검사를 남긴다면 "보조 장치이고 우회 가능하다"를 주석에 반드시 적는다.
행동 테스트에는 **"무엇이 그려졌는가"를 반드시 단언한다** — 클릭만 하고 결과를 안 보면
핸들러를 통째로 지워도 초록이다(실제로 그랬다).
...
| 행 배경을 진하게 했다면 그 위 텍스트 대비를 다시 쟀는가 | 배경을 어둡게 하면 대비가 **낮아진다** |
