...
드러나는** 종류다. 프론트 자동 검증은 `vitest`(`.vue` 밖의 순수 모듈)와
`tools/oracle/front_check.mjs`(파생 계산 대조·소스 가드)뿐이고, 둘 다 컴포넌트를
렌더링하지 않는다 — **컴포넌트 렌더 테스트는 의도적으로 두지 않았다.** 그래서 한 번
밟은 함정은 문서로 남긴다.
...
eslint도 타입 검사도 없고, 백엔드 테스트·오라클·vitest 중 어느 것도 `.vue` 런타임을
실행하지 않는다.
