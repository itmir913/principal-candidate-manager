...
드러나는** 종류다. 프론트 자동 검증은 네 겹이다 — `vitest`(`.vue` 밖의 순수 모듈),
`tools/oracle/front_check.mjs`(파생 계산 대조·소스 가드),
**정적 스캔**(`frontend/tests/undefined-identifiers.test.js`),
**렌더 테스트**(`frontend/tests/smoke-render.test.js`·`quota-input.render.test.js`).
...
- 픽스처가 도달시키지 못하는 화면 영역 전체. `v-if` 뒤에 있으면서 픽스처가 그 조건을
  만들지 못하면 그 영역은 통째로 검사 밖이다. 실측(2026-09-21): 20개 화면 중
  `RENDER_EVIDENCE` 로 "무엇이 그려졌는지"까지 단언하는 것은 4개뿐이고, 나머지는
  "오류 0건"만 본다. `AreasTab` 의 점수표 편집 영역, 담임 지원 입력 폼이 그 예다.
  (`DialogHost` 와 상세 모달은 2026-09-21 에 전용 테스트로 닫았다.)
