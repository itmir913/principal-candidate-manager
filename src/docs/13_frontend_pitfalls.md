...
- 픽스처가 도달시키지 못하는 화면 영역 전체. `v-if` 뒤에 있으면서 픽스처가 그 조건을
  ...
  **19개**(`.vue` 24개 중 props 를 요구하는 5개 제외) 가운데 `RENDER_EVIDENCE` 로
...
이 저장소는 그 실패를 **다섯 번** 겪었다 — F-014(오라클 3c) 1회, F-013(오라클 3d) 2회,
`destructive-confirm` 1회. 매번 "정규식을 넓히면 된다"고 고쳤고 매번 다시 뚫렸다
