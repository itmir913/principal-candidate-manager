...
- 그 학생이 실제로 지원하면 `close_round`의 base_data 누락 검증(`rounds.rs`)에서 422로 다시 막힌다 — 점수가 조용히 0으로 계산되는 경로가 없다.
...
### 42. `src/main.rs` — 트레이 툴팁 제목 조회 실패 시 기본 문구 (`Err(e) => DEFAULT_TITLE`)
작업 표시줄 아이콘의 툴팁에 쓸 제목을 DB에서 읽지 못하면 기본 문구로 대체한다. 표시용 문자열이며
점수·추천·데이터와 무관하고, 여기서 기동을 막으면 DB 일시 오류로 프로그램이 아예 뜨지 않는다.
오류는 버리지 않고 `tracing::warn!` 에 원인까지 남긴다.
**조건**: 트레이 생성 경로에서만. 이 값을 화면·엑셀·감사 로그로 넘기면 위반.

### 43. `src/main.rs::cleanup_legacy_autostart` — 레거시 레지스트리 값 조회 실패 시 정리 건너뜀
0.2.14 이전의 고정 이름 `PCM` 항목을 읽지 못하면 이관을 건너뛴다. 값 없음(`NotFound`)은 정상
경로라 조용히 반환하고, **그 밖의 오류는 `tracing::warn!` 로 남긴다** — 둘을 구별하지 않으면
...
### 46. `src/handlers/app_info.rs` — `unwrap_or_else(|| DEFAULT_*)` (제목·설명 표시)
학교가 앱 제목·설명을 아직 설정하지 않았으면 기본 문구를 보여 준다. 값 없음은 "데이터 오류"가
아니라 **"아직 설정 안 됨"** 이고, 그 사실은 같은 응답의 `configured` 플래그가 별도로 전달한다
(#1 `admin_status` 와 같은 구조). 화면 문구일 뿐 점수·학생 식별·추천 결정과 무관하다.
**조건**: `configured` 가 제목 **행의 존재 여부**로 계산될 것. 이 두 값을 엑셀·감사 로그·
점수 계산으로 넘기면 위반.
