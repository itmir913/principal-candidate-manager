...
`GET /api/audit-logs`의 `AuditRow.actor_ip`로 반환되며, 관리자 감사 기록 화면
(`AuditTab.vue`)의 **상세** 열 끝에 `IP 192.168.0.42` 형태로 붙는다. 엑셀
내보내기(`GET /api/audit-logs/export`)에도 맨 뒤 `IP` 열로 포함된다.
IP를 기록하지 않는 액션은 빈 값으로 남는다.
...
## 담임 로그인 brute-force 방어 (도입하지 않음)

**현재 상태**: 담임 로그인(`teacher_login`)에 rate limit·backoff 방어가 없다.
담임 비밀번호 최소 4자(`classes.rs`, 4자 미만이면 행 오류) + `/api/classes`가 무인증으로
반 목록을 노출하는 조합 때문에, 학교 LAN 안에서 같은 반 학생이 자기 담임 계정을
표적화한 시도가 이론적으로 가능하다.

**소유자 결정 (2026-07-22)**: **도입하지 않는다.** 근거는 bcrypt 검증 비용이다 —
`bcrypt::DEFAULT_COST`(cost 12, 2^12 라운드)로 해시하므로 검증 1회에 수백 ms가 들고,
별도 지연을 넣지 않아도 시도 속도가 구조적으로 제한된다.

**이 근거가 성립하는 범위 (같이 기록해 둘 것)**: bcrypt 비용은 시도 **속도**를 낮출 뿐
탐색 **공간**을 넓히지 않는다. 4자리 숫자 비밀번호(10,000가지)라면 검증 1회 0.3초 기준
순차 시도로도 산술적으로 한 시간 안쪽이고, 동시 요청으로 병렬화하면 더 짧아진다.
따라서 이 결정은 **담임 비밀번호를 충분히 길게 배포한다는 전제**에 의존한다.
방어를 더 원한다면 백오프보다 `classes.rs`의 최소 길이 4자를 올리는 쪽이 효과가 크다.

**참고 — 검토했으나 채택하지 않은 설계**: **지수 백오프**. 응답 자체를 지연시키는 방식.
...
맞춘 결정이다. 방어가 필요하다고 판단되면 백오프·잠금보다 최소 비밀번호 길이를
먼저 검토하라.